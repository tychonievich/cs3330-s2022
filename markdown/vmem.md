...
Also note that this example describes a wasteful system,
in the sense that a page table is 2^8^ bytes and a page is 2^9^ so whatever page has the page table is only half used, a waste of half a page of memory.
We'd expect page tables to be designed to make those two numbers to be the same in any production system to avoid this kind of waste.
:::
