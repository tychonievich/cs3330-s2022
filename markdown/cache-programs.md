...
1.  Download the skeleton code we provide in [this archive](files/cache-progs.tar).

## Lab portion

2.  For the lab, modify or replace the supplied programs `prog1.c` so that, in the simulation under valgrind cachegrind:
...
3.  Upload your `prog1.c` to the submission site. If you collaborated with other students, please
    add a comment at the top explaining who you worked with.

## Homework portion

3.  Modify or replace the supplied programs `prog2.c` and `prog3.c` so that, in the simulation under valgrind cachegrind:
...
2.  Choose **any one** of the programs `prog4.c` and `prog5.c` to modify or replace so that:
    ...
    1.  For `prog5.c`: Write a program that performs at least 10 million cache references and achieves a <40% miss rate with 32KB, 2-way data cache with 128B blocks but a >50% miss rate with a 32KB, 2-way data cache with 64B blocks. Submit your program as `prog5.c`
...
*  prog1.c, prog2.c, prog3.c, prog4.c, prog5.c --- templates for the programs you need to produce below. The
...
this pattern in by changing some settings in the program.
